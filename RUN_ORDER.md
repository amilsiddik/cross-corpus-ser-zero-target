...
The output is given a new suffix, `plus_base`, so it does not overwrite the previous folders.
...
`08_train_cross_attention_fusion_intra.ipynb` was also fixed, but this model is not a top priority if the manuscript only uses Sequence-level Cross-Attention.
...
## Important File for revision/reviewer
